    ...
    <div class="p-2 absolute top-0 left-0 right-0 flex items-start md:items-center justify-between z-10"><span
        ...
        <div class="flex items-center flex-col md:flex-row gap-1"><span
                ...
                class="bg-warning text-white rounded-full py-1 px-2.5 md:px-3 md:py-1.5 flex items-center text-sm gap-1"><img
                    data-src="{{ url('assets/icons/clock_icon.svg') }}" class="w-6 h-6 lazyload"> 15-20 min </span></div>
    </div>
    <div class="flex flex-col gap-2">
        <div class="flex md:items-center items-start justify-between flex-col md:flex-row gap-1.5 mt-1.5 md:mt-3 mb-1">
            <span class="flex items-center capitalize gap-1.5 text-muted">
                <img alt="Fast Ship Hu" onerror="this.onerror=null; this.src='{{ url('images/no-image.png') }}'" class="w-7 h-7 rounded-full lazyload" width="28" height="28" data-src="{{ url(optional($itemPF->store)->avatar_image) }}">{{ optional($itemPF->store)->name }} </span> <span
        ...
        <div class="flex flex-wrap items-center justify-between font-medium">
            <div class="flex flex-wrap items-center gap-1 text-base md:text-lg"><span
            ...
            <div class="flex items-center gap-2 text-gray-400"><img alt="Fast Ship Hu"
                    ...
                    class="w-6 h-6 lazyload"><span>{{ \App\Models\Order::getDistance($_COOKIE['lat'] ?? 47.1611615, $_COOKIE['lng'] ?? 19.5057541, optional($itemPF->store)->lat, optional($itemPF->store)->lng)['distance_km'] }} km</span>
            </div>
        </div>
    </div>
</div>
