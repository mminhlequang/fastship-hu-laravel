    ...
    <div class="box">
        <div class="content-header border-bottom pb-5">
            ...
            @can('UsersController@store')
                <a href="{{ url('/admin/users/create') }}" class="btn btn-default float-right"
            ...
            <div class="box-tools">
                ...
                <div class="input-group" style="margin-right: 5px; display:flex;">
                    <div class="select-group" style="margin-right: 5px; width:206px;">
                        {!! Form::select('role_id', $roles ?? [], \Request::get('role_id'), ['class' =>'form-control input-sm select2 ','id' => 'role_id']) !!}
                    </div>
                    <input type="text" value="{{\Request::get('search')}}" class="form-control input-sm" name="search"
                           placeholder="{{ __('message.search_keyword') }}" autocomplete="off"
                                    ...
                                    <a href="{{ url('/admin/users/' . $item->id) }}"
                                       title="{{ __('message.user.view_user') }}">
                                        <button class="btn btn-info dropdown-item"><i
                                                    class="fas fa-eye"></i> {{ __('message.user.view_user') }}</button>
                                    </a>
                                @endcan
                                    ...
                                    <a href="{{ url('/admin/users/' . $item->id . '/edit') }}"
                                       title="{{ __('message.user.edit_user') }}">
                                        <button class="btn btn-primary dropdown-item"><i class="far fa-edit"
                                                                                         aria-hidden="true"></i> {{ __('message.user.edit_user') }}
                                        </button>
                                    </a>
                                @endcan
