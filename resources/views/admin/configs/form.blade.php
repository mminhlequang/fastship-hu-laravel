    ...
    <table class="table table-striped">
        <tr class="row {{ $errors->has('user_id') ? 'has-error' : '' }}">
            ...
            <td class="col-md-8 col-lg-9">
                {!! Form::select('user_id', $users ?? [], isset($data) ? $data->user_id : \Auth::id(),['class' => 'form-control input-sm select2', 'id' => 'selectUser']) !!}
            ...
            <tr class="row">
                ...
                <td>
                    <div class="{{ $errors->has('name') ? ' has-error' : ''}}">
                        {!! Form::text('input[0][text]', null, ['class' => 'form-control input-sm', 'placeholder' => 'Tên trường']) !!}
                    </div>
                </td>
                <td>
                    <div class="{{ $errors->has('year_id') ? ' has-error' : ''}}">
                        {!! Form::select('input[0][type]', \App\Models\Config::$TYPE ?? [], null, ['class' => 'form-control input-sm select2']) !!}
                    </div>
                </td>
                <td>
...
@section('scripts-footer')
    @if(!\Auth::user()->isAdminCompany())
        ...
        <script>
        $('#selectUser').on('select2:opening select2:closing', function(event) {
                event.preventDefault();
        });
        $('#selectUser').addClass('select2-container--readonly');
        </script>
    @endif
