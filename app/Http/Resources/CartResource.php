...
class CartResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $productss = Product::where('id',$this->product_id)->first();
        ...
        return [
            ...
            'product' => [
                ...
                'name' => trim($name12 ?? "" . ' ' . ($size ? 'Size ' . $size . ' ' : '') . ($pack ? 'Đóng gói ' . $pack . ' ' : '') . ($material ? 'Vật liệu ' . $material . ' ' : '') . ($color ? 'Màu ' . $color . ' ' : '') . ($weight ? 'Khối lượng ' . $weight . ' ' : '')),
                'image' => optional($this->product)->image ? asset(optional($this->product)->image) : '',
                'price' => $properties->price ?? 0,
                'percent' => optional($this->product)->percent,
